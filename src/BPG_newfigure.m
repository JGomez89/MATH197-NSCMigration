...
%% For storing distance at each step
...
% 
%     for i = 1:n_seeds
%         dist2cancer(i) = sqrt((cancer_center(1) - p(i).coord(1,Finaltimestep)).^2 + (cancer_center(2) - p(i).coord(2,Finaltimestep)).^2 + (cancer_center(3) - p(i).coord(3,Finaltimestep)).^2);
%     end 
% 
%     % close to cancer distance less than # = 100
%     cellind = find(dist2cancer < 100);
% 
...
% Plot regular graph
xvals = (1:Finaltimestep); % *round(Finaltimestep/acc)
...
if has_cancer
    ...
    for i = 1:Finaltimestep
        numAtCancer = 0;
        for j = 1:n_seeds
            if abs( cancer_center(1) - p(j).coord(1,i) ) < cancer_sd(1)          && ...
                    abs( (cancer_center(2) - p(j).coord(2,i)) ) < cancer_sd(2)   && ...
                    abs( (cancer_center(3) - p(j).coord(3,i)) ) < cancer_sd(3)
...
%   concentration(concentration > valcap) = valcap;
    ...
    valcap = 1./(1+(sqrt(((cancer_sd_X)/concen_sd(1)).^2+((0)/concen_sd(2)).^2+((0)/concen_sd(3)).^2)).^concen_param);
    concentration(concentration > valcap) = valcap;
