...
n_seeds = 250;                                                                                          % Total number of paths generated
WM_path = double(WM);  
    ...
    for i = 2:2000                                                                                    % Run the loop a large number of steps
        ...
        if ~flag1                                                                                       % Positive direction path
            ...
            if coh_map(ind1)                                                                            % Get Eigen vector angle at the previous coordinate point
                ...
                %FIX ME
                % Choose +/- depending on positive inner product (See scratch paper)
                cancer_vect = [cancer_center(1) - seednow(1), cancer_center(2) - seednow(2)];
                
                angle1 = ori_map(ind1);
                temp_eigen_vect = [sind(angle1),cosd(angle1)];

                if(dot(temp_eigen_vect,cancer_vect) < 0)
                    angle1 = angle1 + 180;
                end
                
                p(seed_loop).angle1(i) = angle1;                
                
            else
                angle_sampled = -90+rand*180;
                angle1 = rem(angle_sampled + p(seed_loop).angle1(i-1),360);                              % Real value of angle if sampled angle + original angle > 360 deg
           ...
           gradVect = [cgradY(seednow(1,1),seednow(1,2)) , cgradX(seednow(1,1),seednow(1,2))];
           gradVect = gradVect / norm(gradVect);
                      
           angle2 = acosd(gradVect(1))-90; % subtract 90 since initially at +90 degrees
           
           w = rand(1);
           stochAngle = w*angle1 +(1-w)*angle2;
           
           vectLeng = rand(1) * d;  %Max vetor length
           eigen_vect = vectLeng * [sind(stochAngle),cosd(stochAngle)];                                                     % Calculate the eigen vector (EV) direction

           % +/- dependent on initial choice of angle
           p(seed_loop).coord1{i} = p(seed_loop).coord1{i-1} + eigen_vect;                           % Find next coordinate point at step d along EV direction
...
% 
%                 angle1 = -ori_map(ind2);
%                 p(seed_loop).angle2(i) = angle1;                
%                 
%             else
%                 angle_sampled = -90+2*rand*90;
%                 angle1 = rem(angle_sampled + p(seed_loop).angle2(i-1),360);                              % Real value of angle if sampled angle + original angle > 360 deg
%                 p(seed_loop).angle2(i) = angle_sampled;
%             end
%             
%             p(seed_loop).coherency2(i) = coh_map(ind2);
%             eigen_vect = [sind(angle1),cosd(angle1)];                                                     % Calculate the eigen vector (EV) direction
%             
%             
%             if pRand <= pStay
