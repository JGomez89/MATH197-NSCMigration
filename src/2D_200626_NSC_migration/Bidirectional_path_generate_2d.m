...
%inj_center = [5100,730];
seed_ROI = sqrt((X-inj_center(2)).^2 + (Y-inj_center(1)).^2)<500;   %% seed: circle around inj_center                                     % All points within 500 pixel distance from inj_center
...
can_center = [1000,730]; 
sig = 1250; 
concentration = exp( ( -(X-can_center(2)).^2 -(Y-can_center(1)).^2 )./sig^2 ); 

CgradX = zeros(size(concentration)); 
CgradY = zeros(size(concentration)); 
...
seed_loop
    seed = seed_ind(round(rand*length(seed_ind)));    %% choose random among seed_ind                                                    % Choose a random point index to start the simulation
    ...
    for i = 2:90000                                                                                    % Run the loop a large number of steps

        if ~flag1                                                                                       % Positive direction path
            
            ind1 = sub2ind(size(WM),round(p(seed_loop).coord1{i-1}(1)),round(p(seed_loop).coord1{i-1}(2)));         % Index of previous coordinate point
...
% some indices had some issues. Not sure what at this point. Just keeping that part of the code in as commented.    
%     if any(i == edit_indices)
%         path_temp = reshape(cell2mat(p(i).coord1),2,[]);
%         len = size(path_temp,2);
%         min_ind = min(find((path_temp(1,:)>5620).*(path_temp(2,:)<500)));
%         path1 = path_temp;
%         path1(:,min_ind+1:end) = repmat(path_temp(:,min_ind),1,len-min_ind);
%         X_coords(1:len,i) = path1(1,:)';
%         Y_coords(1:len,i) = path1(2,:)';
%     else
