...
if( ~exist('coh_map','var') ) 
    WM = imread(strcat('WM_M43.tif'));
    WM = logical(WM.*(WM>10));                                                                              % Threshold image to remove background
    ori_map = 180*imread(strcat('Orientation_5vox_blur.tif'))/pi;   % -90 to 90                                    % Orientation map converted to degrees (-90 to 90 deg)
    ...
    coh_map = imread(strcat('Coherency_5vox_blur.tif')).*WM;
...
%stochasticity in IC of NSC 
    dmax  = 5; 
    dstochastic = 5; %test 1-8
    beta4dist = 2; %test 1-5
...
for num_time_step = 1:Finaltimestep/3
    curr_error = 0;
    for i = 1:3
        curr_error = curr_error + abs((meanVals(i*num_time_step) - compare_data(i))/compare_data(i));
    end
    
    if (curr_error < min_error)
        min_error = curr_error;
        best_fit_step = num_time_step;
    end
end
