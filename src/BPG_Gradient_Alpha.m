    ...
    for i = 3:Finaltimestep                                                                                 % Run the loop a large number of steps            
        ...
        elseif modelNum == 2
            ...
            if has_cancer && (concentration(seedx,seedy) >= chmtx_limit)
                stochastic_chemosensitivity = betainv(rand(1),alpha4chmtx,1) * chemo_sensitivity;
            else
                stochastic_chemosensitivity = 0;
            end
            
            p(seed_loop).coord(:,i) = p(seed_loop).coord(:,i-1) + dstep*eigen_vect + stochastic_chemosensitivity*chmtx_vect;
